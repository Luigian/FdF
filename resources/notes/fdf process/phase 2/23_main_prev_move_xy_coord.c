...
/*   Updated: 2019/09/06 21:10:13 by lusanche         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		*store_x_and_y_coordenates(t_node *a, t_node *b, void **param)
{
	...
	map = (t_map *)(param[3]);
	xy[0] = a->x;
	xy[1] = a->y - (a->z * (map->up * Z_SPEED));
	xy[2] = b->x;
	xy[3] = b->y - (b->z * (map->up * Z_SPEED));
	return (xy);
}

void	draw_line(void **param, t_node *a, t_node *b)
{
	int		*xy;
	int		*d;
	int		*s;
	...
	map = (t_map *)(param[3]);
	xy = store_x_and_y_coordenates(a, b, param);
