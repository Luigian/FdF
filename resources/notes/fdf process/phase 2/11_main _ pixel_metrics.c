...
/*   Updated: 2019/09/04 15:47:07 by lusanche         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	assign_coordenates(t_node *trav, double x, double y, double *pixels)
{
	t_node	*start;
	t_node	*adv;
	double		adj;
...
double		*pixel_metrics_calculator(int rows, int cols/*, int *zaxis*/)
{
	double		*pixels;
	int			sides;
	double		adj;
...
int		main(int ac, char **av)
{
	...
	assign_coordenates(beg, WIN_WIDE / 2, WIN_HEIGH - (WIN_WIDE / 16), pixels);
	printf("end\n");
	mlx_ptr = mlx_init();
