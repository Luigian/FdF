...
/*   Updated: 2019/09/08 21:56:43 by lusanche         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		*store_x_and_y_coordenates(t_node *a, t_node *b, void **param)
{
	...
	map = (t_map *)(param[3]);
	xy[0] = a->x + ((((map->xs - a->x) * ZOOM_SPEED) / 100) * map->zoom);
	xy[1] = a->y + ((((map->ys - a->y) * ZOOM_SPEED) / 100) * map->zoom);
	xy[2] = b->x + ((((map->xs - b->x) * ZOOM_SPEED) / 100) * map->zoom);
	xy[3] = b->y + ((((map->ys - b->y) * ZOOM_SPEED) / 100) * map->zoom);
	//////////////////////////////////////////////////////////////////
	xy[0] += (map->x * X_Y_SPEED);
	xy[1] += (a->z * (map->z * Z_SPEED)) + (map->y * X_Y_SPEED);
	xy[2] += (map->x * X_Y_SPEED);
	xy[3] += (b->z * (map->z * Z_SPEED)) + (map->y * X_Y_SPEED);
