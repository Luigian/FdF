...
/*   Updated: 2019/09/02 22:08:16 by lusanche         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_grid(void *mlx_ptr, void * win_ptr, t_node *trav)
{
	...
	while (trav)
	{
		while (adv->right)
		{
			...
			if (adv->up)
				draw_line(mlx_ptr, win_ptr, adv, adv->right->up);
			adv = adv->right;
		}
		trav = trav->up;
		adv = trav;
	...
	while (trav)
	{
		while (adv->up)
		{
			draw_line(mlx_ptr, win_ptr, adv, adv->up);
			adv = adv->up;
		}
		trav = trav->right;
		adv = trav;
...
void	connect_up(t_node *trav_1, t_node *trav_2)
{
	trav_2->up = trav_1;
	while (trav_2->right)
	{
