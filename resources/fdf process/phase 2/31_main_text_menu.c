...
/*   Updated: 2019/09/09 11:57:38 by lusanche         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
*/		map->on = 0;
	...
	if (key == 78)
	{
/*		if ((map->z - map->zoom) > 10 && map->win_heigh != 1440)
		{
			map->win_heigh = 1440; 
			map->win_wide = 2560;
			map->x += 10;
...
*/		map->on = 0;
	...
	if (key == 91)
	{
/*		if (map->zoom < -2 && map->win_heigh != 1440)
		{
			map->win_heigh = 1440; 
			map->win_wide = 2560;
			map->x += 10;
...
void	store_x_and_y_coordenates_front(t_node *a, t_node *b, void **param, t_co *co)
{
	...
	beg = (t_node *)(param[2]);
	map = (t_map *)(param[3]);
	co->x0 = a->a + ((((map->xs - a->a) * ZOOM_SPEED) / 100) * map->zoom);
	co->y0 = a->b + ((((map->ys - a->b) * ZOOM_SPEED) / 100) * map->zoom);
	co->x1 = b->a + ((((map->xs - b->a) * ZOOM_SPEED) / 100) * map->zoom);
	co->y1 = b->b + ((((map->ys - b->b) * ZOOM_SPEED) / 100) * map->zoom);
	co->x0 += (map->x * X_Y_SPEED);
	co->y0 += (a->z * (map->z * Z_SPEED)) + (map->y * X_Y_SPEED);
	co->x1 += (map->x * X_Y_SPEED);
	co->y1 += (b->z * (map->z * Z_SPEED)) + (map->y * X_Y_SPEED);
...
void	draw_grid(void **param)
{
	...
	mlx_string_put(param[0], param[1], 30, 50, 0xFFFFFF,  "Altitud  : + and -");
	mlx_string_put(param[0], param[1], 30, 70, 0xFFFFFF,  "Position : arrow keys");
	mlx_string_put(param[0], param[1], 30, 90, 0xFFFFFF,  "Zoom     : mouse scroll");
