...
/*   Updated: 2019/09/06 10:55:39 by lusanche         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	erase_canvas_1(void **param)
{
	t_node		*trav;
	t_node		*beg;
...
void	erase_canvas_2(void **param)
{
	int		x;
	int		y;
	...
	while (x <= WIN_WIDE)
	{
		y = 0;
		while (y <= WIN_HEIGH)
			mlx_pixel_put(param[0], param[1], x, y++, 0);
		++x;
	}
}

void	erase_canvas_3(void **param)
{
	t_node		*trav;
	t_node		*beg;
...
void	draw_grid(void *mlx_ptr, void * win_ptr, t_node *trav)
{
	...
	trav = move_pointer_to_top_right_corner(trav);
	adv = trav;
